...
def preprocessing_image_rgb(x):
    ...
    # Updated Values
    mean = [191.189, 194.699, 198.286]
    std = [81.472, 77.442, 74.553]
...
def preprocessing_image_ms(x):
    # define mean and std values
    mean = [1116.468, 1041.475, 945.344, 1198.498, 2004.878,
            2376.699, 2303.738, 732.957, 1116.271, 2602.579]
    std = [154.008, 187.997, 278.508, 228.122, 356.598, 456.035,
           531.570, 98.947,303.851, 503.181]
    # loop over image channels
    for idx, mean_value in enumerate(mean):
        x[..., idx] -= mean_value
        x[..., idx] /= std[idx]
